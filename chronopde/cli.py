...
def diagnose_loss_alignment_main(argv: Sequence[str] | None = None) -> int:
    """Run the Week 6 full-field-relative loss diagnostic."""

    parser = argparse.ArgumentParser(description="Run the Week 6 loss-alignment diagnostic.")
    parser.add_argument("--config", default="configs/project.yaml")
    parser.add_argument("--data-path")
    parser.add_argument("--device", choices=("auto", "cpu", "cuda", "mps"), default="auto")
    parser.add_argument("--max-steps", type=int, default=5_000)
    ...
    payload: dict[str, object] = {
        "command": "diagnose_week6_loss_alignment",
        "data_path": str(data_path),
        "device": args.device,
        "training": True,
        "production_loss_changed": False,
        "seed": 0,
        "trajectory_ids": [f"train-{index:04d}" for index in range(4)],
        "interval_indices_per_trajectory": [0, 33, 66, 99],
        "batch_size": 16,
        ...
        "minimum_loss_reduction": 1_000,
        "median_velocity_nrmse_threshold": 0.01,
        "output_directory": str(root / "artifacts/diagnostics/week6/loss_alignment"),
    }
    if args.dry_run:
        print_payload(payload)
        return 0
    from chronopde.diagnostics import run_loss_alignment_suite

    report = run_loss_alignment_suite(
        config,
        root,
        data_path,
        device_name=args.device,
